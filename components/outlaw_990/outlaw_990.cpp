...
Outlaw990::Outlaw990() :
  PollingComponent(1000), rx_state(0), polling_enabled(true), powering_off(false)
{
}
...
void Outlaw990::update() {
  if (polling_enabled || powering_off) {
    ESP_LOGD(TAG, "update() polling %s powering_off %s", polling_enabled ? "enabled" : "disabled", powering_off ? "true":"false");
    send_pkt(0x53);
  }
}
...
void Outlaw990::loop() {
  ...
  switch (rx_state) {
    case RX_STATE_INIT:
      ...
      send_idle_values();
      rx_state = RX_STATE_HEADER_BYTE1;
      break;

    case RX_STATE_HEADER_BYTE1:
      if (available()) {
        if (read() == 0x83) {
          rx_state = RX_STATE_HEADER_BYTE2;
        }
      }
      break;
    ...
    case RX_STATE_PAYLOAD:
      if (available() >= 25) {
        ...
        if (checksum == rx_buf[24]) {
          rx_state = RX_STATE_PARSE;
        } else {
          ESP_LOGD(TAG, "Checksum error, calc=%2.2X rx=%2.2X", checksum, rx_buf[24]);
          rx_state = RX_STATE_INIT;
        }
